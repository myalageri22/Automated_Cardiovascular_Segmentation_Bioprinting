...
TotalSegmentator v1.5.5 was used only as an additional tool for cardiac-region localization or anatomical context during preprocessing and post-spatial organization \cite{wasserthal2023}. It was not used to generate coronary artery labels, not treated as a microvascular reconstruction method, and not used as evidence of capillary-scale vascular modeling. This distinction is important because the supervised vascular output of this study comes from the ImageCAS-trained coronary segmentation model, not from TotalSegmentator. TotalSegmentator outputs did not enter any reported segmentation metric, mesh-QC value, or STL export.
...
To evaluate segmentation-derived STL outputs for downstream fabrication-oriented review, the Phase B workflow included computational checks of mesh integrity, cross-sectional slicability, and spatial fidelity. Mesh integrity was evaluated using watertightness, detected non-manifold edges, and connected-component count. A mesh passed the combined integrity criterion when it was watertight and had zero detected non-manifold edges after repair; completion of the repair routine was recorded separately from passing this criterion.
...
To distinguish these geometric checks from production-slicer execution, repaired Phase B STL outputs for all 250 held-out test cases were processed using PrusaSlicer 2.9.6 with a fixed Original Prusa MK4S computational reference configuration, Generic PLA, a \SI{0.4}{mm} nozzle, the \texttt{0.20mm STRUCTURAL @MK4S 0.4} preset, two perimeters, 15\% infill, and \SI{0.20}{mm} layer height. Native 100\% scale and the source STL orientation were retained, with automatic organic supports enabled everywhere. Toolpath-generation success was predefined as a zero PrusaSlicer exit status, a nonempty textual G-code output, at least one detected printed layer, and at least one spatial movement with positive extrusion. Complete-STL processing, native-scale reference build-volume fit, support-path generation, warnings, and per-case results were retained together with the fixed configuration, hashes, command template, and version record. No physical printer or physical fabrication was used.

\begin{table}[!htbp]
\centering
\caption{Computational geometry and spatial-fidelity checks applied to the reported Phase B outputs.}
...
\midrule
Combined mesh integrity & Requires a watertight repaired surface with zero detected non-manifold edges. \\
Connected components & Quantifies segmentation-derived fragmentation retained in the repaired mesh. \\
Primary slicability & Requires closed contours on every plane that intersects the mesh. \\
Strict slicability & Requires an intersection with closed contours on all 50 requested planes. \\
World-coordinate bounding-box alignment & Compares mask and mesh bounds after full-affine RAS mapping; the pass tolerance is two voxels. \\
Centroid displacement & Measures Euclidean mask-to-mesh centroid offset in millimeters. \\
\bottomrule
\end{tabularx}
\end{threeparttable}
\end{table}
...
\begin{table}[!htbp]
\centering
\caption{Cohort-level Phase B mesh-quality results over the 250-case held-out test set, read from \texttt{outputs/phase\_b\_mesh\_qc/summary\_mesh\_qc.json}, \texttt{per\_case\_mesh\_qc.csv}, and \texttt{missing\_checks\_per\_case\_traceable\_v14.csv}.}
\label{tab:phaseb-qc}
\begin{threeparttable}
\footnotesize
...
For tubular vascular anatomy, topology-sensitive measures such as clDice are especially relevant because they emphasize centerline overlap and connectedness more directly than volumetric overlap alone \cite{shit2021}. The held-out test evaluation computed clDice@0.5 over all 250 cases on the saved per-case predictions, yielding a mean of 0.8695 (95\% CI 0.8629-0.8761; median 0.8753; SD 0.0531; range 0.5764-0.9744). clDice@0.5 (0.8695) was higher than Dice@0.5 (0.7878). Because the two metrics are not on the same scale, we do not read this gap as a direct comparison; we report clDice because centerline connectivity matters for fabrication and Dice can miss it. clDice used the soft-skeleton definition of Shit et~al.~\cite{shit2021}: binary skeletons were computed with scikit-image \texttt{skimage.morphology.skeletonize} on the \SI{0.6}{mm} isotropic evaluation grid, with the ground-truth label resampled onto the prediction grid via \texttt{nibabel.processing.resample\_from\_to} (order~0, nearest-neighbour) and both volumes binarized at threshold 0.5. Topology precision $T_{\mathrm{prec}}$ and sensitivity $T_{\mathrm{sens}}$ were combined as $\mathrm{clDice}=2\,T_{\mathrm{prec}}T_{\mathrm{sens}}/(T_{\mathrm{prec}}+T_{\mathrm{sens}})$.
...
Full-cohort production-slicer validation evaluated all 250 repaired held-out test STLs. All 250 executions returned exit status zero, generated nonempty textual G-code, and met the predefined toolpath-generation criteria (250/250, 100.0\%), with zero failures. Every complete STL fit within the reference build volume at native scale, and organic support paths were generated for all 250 cases. The median layer count was 450 (IQR 423--489; range 309--677), and the median number of positive-extrusion movements was 174{,}104 (range 45{,}667--317{,}816). Median estimated time was 7{,}488.5~s (approximately 2~h 4~min 49~s; IQR 6{,}307.25--8{,}725.5~s; range 2{,}568--13{,}419~s). Cases 803, 821, and 822 each produced an empty-layer warning, and the remaining 247 cases were warning-free. All 22 meshes that failed the earlier combined mesh-integrity criterion nevertheless met the production-slicer toolpath criteria. Table~\ref{tab:production-slicer-validation} summarizes these results. No physical fabrication was performed.

\begin{table}[!htbp]
\centering
\caption{Full-cohort production-slicer validation of repaired Phase B STL outputs from the 250-case held-out test set.}
...
Preset and processing & \texttt{0.20mm STRUCTURAL @MK4S 0.4}; \SI{0.20}{mm} layers; two perimeters; 15\% infill; native 100\% scale; source orientation retained \\
Successful toolpaths & 250/250 (100.0\%); zero failures \\
Complete STL processed / build-volume fit & 250/250 / 250/250 \\
Organic support paths generated & 250/250 \\
Warning-free / warned cases & 247 / 3 (cases 803, 821, and 822; empty-layer warnings) \\
Layers & Median 450; IQR 423--489; range 309--677 \\
Positive extrusion movements & Median 174{,}104; range 45{,}667--317{,}816 \\
Estimated time & Median 7{,}488.5~s; IQR 6{,}307.25--8{,}725.5~s; range 2{,}568--13{,}419~s \\
Earlier mesh-integrity failures meeting slicer criteria & 22/22 \\
...
\\
Cohort production-slicer validation (250 cases)
&
250/250 (100.0\%) met the predefined toolpath-generation criteria under one fixed PrusaSlicer profile; 247 cases were warning-free and 3 produced empty-layer warnings; median 450 layers and 174{,}104 positive-extrusion movements; no physical fabrication
\\
...
The pipeline connects coronary CTA segmentation to fabrication-oriented mesh generation. The full 250-case held-out ImageCAS evaluation supports the segmentation stage. The primary Dice@0.5 result was 0.7878 with a 95\% CI of 0.7820-0.7935, median 0.7934, and zero failed cases, with a topology-aware clDice@0.5 of 0.8695. The mean Dice was just below 0.80. The paired analysis shows centerline-aware accuracy predicts mesh connectivity better than voxel overlap. The full-cohort slicer assessment adds software-level evidence that all 250 repaired held-out test STLs can generate textual toolpaths under one fixed computational reference profile.

\FloatBarrier

The main implication is that image-to-fabrication pipelines should not end just at segmentation metrics. For coronary and vascular structures, distal branch dropout, false bridges, and local discontinuities may have limited effect on Dice but substantially affect geometry. This is especially important for biofabrication, where a disconnected or non-manifold vascular model may fail as a perfusable blueprint even if there is high voxel overlap. The representative Phase B result complements this claim but in an opposite direction, showing that a mesh can be geometrically clean (watertight and single-component) yet fail the stated computational minimum-radius proxy because much of coronary geometry is at a sub-millimeter scale. Both observations argue that segmentation accuracy and fabrication readiness must be reported and evaluated as distinct objectives. The present pipeline addresses that gap by generating STL artifacts and QC summaries. The full-cohort PrusaSlicer assessment provides supporting computational evidence that all repaired held-out test STLs reached textual toolpaths under one fixed reference profile. The 22 meshes that failed the combined mesh-integrity criterion nevertheless met the production-slicer criteria, confirming that geometric integrity checks and production-slicer execution measure different properties and that successful execution does not override geometric QC failures. The clDice--mesh-fragmentation relationship remains the principal scientific finding.
...
The corrected cohort checks separate geometric integrity (watertightness and detected non-manifold edges), fragmentation, cross-sectional slicability, and affine-aware spatial fidelity. The full-cohort production-slicer assessment separately evaluates software-level toolpath generation under one fixed profile. These distinct checks form an intermediate computational layer between voxelwise segmentation and physical fabrication without establishing physical printability.

The claim boundary is therefore explicit. These results demonstrate computational preparation of STL geometry from CTA-derived vascular segmentations and software-level STL-to-toolpath generation under one fixed reference profile. They do not demonstrate physical printing, cell-laden printing, bioink compatibility, quantitative print fidelity, mechanical performance, lumen patency, perfusion under flow, endothelialization, long-term construct viability, implantation, or clinical readiness. Reaching those endpoints requires bioprinter-specific toolpath validation, physical print trials, and material, mechanical, perfusion, and biological testing.


\subsection{Limitations}
Several limitations should be stated explicitly. First, the dataset labels coronary arteries, not the full cardiac vascular hierarchy. The system is therefore a coronary (macrovascular) CTA-to-STL pipeline, not a microvascular reconstruction method. Second, the study is entirely computational and reports no physical bioprinting, mechanical testing, perfusion assay, or biological validation. Third, evaluation is confined to a single dataset (ImageCAS), and external validation on independent coronary CTA data is still needed. Fourth, although clDice and the paired analysis show that topology tracks mesh connectivity, overlap-based metrics alone, voxel or centerline, cannot establish distal-branch continuity, lumen preservation, or print readiness. Fifth, although full-cohort production-slicer validation was completed under one fixed PrusaSlicer reference profile, bioprinter-specific toolpath validation and physical fabrication were not performed. Bioink compatibility, lumen patency, perfusion, mechanical behavior, quantitative surface fidelity, and biological function were not evaluated. Sixth, component counts above the two-vessel anatomical baseline were common (median 9), which is consistent with the clDice-fragmentation relationship in Section~\ref{subsec:paired}. Connected-component filtering, already supported in the implementation but not applied here, is a straightforward next step toward single-tree exports.
...
The full-cohort PrusaSlicer validation establishes software-level toolpath generation for all 250 repaired held-out test STLs under one fixed reference profile. Remaining work includes bioprinter-specific toolpath generation, bioink- and material-aware parameterization, physical printing, quantitative geometric print-fidelity testing, and mechanical, perfusion, and biological evaluation where appropriate. External validation should also accompany these studies.

% =========================================================
\section{Conclusion}
% =========================================================

This study presents an integrated computational pipeline that converts coronary CT angiography into segmentation-derived, bioprinting-oriented vascular STL models. On the complete 250-case held-out ImageCAS test set, the three-dimensional Attention U-Net achieved a mean Dice@0.5 of 0.7878 and a mean topology-aware clDice@0.5 of 0.8695, with successful inference across all cases. The Phase B workflow completed mesh extraction and repair for the full cohort, with 91.2\% meeting the combined watertight and zero-detected-non-manifold-edge criterion and all 250 meshes meeting the two-voxel affine-aware alignment criterion.
...
By combining coronary segmentation, topology-aware evaluation, mesh reconstruction, repair, and automated quality-control reporting in a single reproducible framework, the pipeline covers the image-to-mesh steps between medical imaging and biofabrication that usually require manual work. Full-cohort production-slicer validation also generated successful textual toolpaths for all 250 repaired held-out test STLs under one fixed reference profile, without physical printing. The workflow is intended to support fabrication planning, sacrificial-channel design, and later experimental validation of image-derived vascular geometries.

\section*{Data and code availability}

The completed held-out test outputs are retained in \texttt{outputs/final\_test\_250/} with per-case results, summary metrics, evaluation logs, configuration files, and paired-analysis outputs. Corrected cohort mesh-quality, slicability, centroid, and alignment results are retained in \texttt{outputs/phase\_b\_mesh\_qc/}; \texttt{run\_missing\_checks.py} reproduces the lightweight checks when the excluded case-level masks and meshes are available. The full-cohort production-slicer workflow is implemented in \texttt{production\_slicer\_validation.py}; its publication bundle is retained in \texttt{outputs/production\_slicer\_validation/cohort\_250\_real/} as \texttt{per\_case\_slicer\_results.csv}, \texttt{summary\_slicer\_results.json}, \texttt{validation\_manifest.json}, and the verification report. Only the \texttt{cohort\_250\_real} outputs are used for the reported 250-case production-slicer statistics. The separate case-720 files remain an illustrative validation-partition example. The evaluation can be reproduced from the checkpoint \texttt{checkpoints/best\_dice05.pt} using the orchestration script \texttt{run\_full\_revision.sh}. Together, these artifacts support verification of the reported held-out test, clDice, mesh-QC, paired-analysis, and full-cohort production-slicer results. The per-epoch training log for the evaluated checkpoint was not retained (Section~\ref{subsec:training-convergence}).

The implementation was developed in Python using PyTorch, MONAI, NumPy, SciPy, NiBabel, pandas, and related medical image-processing libraries. To support double-anonymous review, the manuscript, code, and derived results were anonymized. An anonymized code and derived-results archive is available through the review repository listed in the Code Availability appendix, subject to the original ImageCAS dataset-use conditions. User-specific paths, usernames, email addresses, and institutional identifiers were removed from the codebase, logs, figures, and manuscript materials.
...
The corrected cohort-level Phase~B artifacts separately report connected-component count, the combined watertight and zero-detected-non-manifold-edge criterion, primary and strict slicability, centroid displacement, and affine-aware bounding-box alignment.

The retained production-slicer cohort is verified by running \texttt{python verify\_production\_slicer\_cohort.py} from the repository root. The script checks cohort identity, success criteria, warnings, inclusive quartiles, profile and artifact hashes, summary/manifest consistency, and the join to the 22 Phase~B mesh-integrity failures. Full G-code and case-level STL payloads are intentionally excluded from the publication repository; their hashes and compact parsed results are retained.
