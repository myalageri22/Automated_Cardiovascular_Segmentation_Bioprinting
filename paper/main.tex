...
\begin{figure}[!t]
    ...
    \caption{Representative preprocessing panel showing raw CTA, windowed CTA, resampled CTA, and the ground-truth vessel overlay prepared for model ingestion.}
    \label{fig:preprocessing}
    \TODO{regenerate this figure before submission: the current export carries an authoring ``Poster tip'' annotation and raw \texttt{np.float32(...)} debug text in the panel titles, and shows more panels (probability map, ground-truth overlay, case-level summary) than the caption describes. Remove the poster note/debug text and align the panels with the caption.}
\end{figure}
...
The fully documented hyperparameter set above corresponds to the exported \texttt{bioprint\_v11} run, which provides the training- and validation-history records summarized in Section~\ref{subsec:training-convergence}. The checkpoint actually evaluated on the held-out test set was a weight-averaged ``soup'' from a later Tversky--Dice run sharing the same Attention U-Net architecture; its full training-run folder was not retained (Section~\ref{subsec:training-convergence}). An accountable checkpoint (\texttt{checkpoints/best\_dice05.pt}; epoch 79; 23{,}631{,}875-parameter Attention U-Net; validation Dice@0.5 0.7889) is, however, retained and is the planned source for re-evaluation. \TODO{after re-evaluating the held-out test set from \texttt{best\_dice05.pt} (orchestrated by \texttt{run\_full\_revision.sh}), reconcile this paragraph so the reported test metrics and the documented training run refer to the same checkpoint.}
...
\begin{figure}[!t]
    ...
    \TODO{the best validation value shown in this figure ($\approx$0.797) was generated from a run that differs from the v11 history quoted in Section~\ref{subsec:training-convergence} (best 0.7542 at epoch 73). After re-evaluating from the accountable checkpoint, regenerate this figure and reconcile the validation numbers across the text and this caption.}
\end{figure}
...
A representative vessel-network STL rendering from the geometry stage is shown in figure~\ref{fig:vessel-stl}, and a representative case-level Phase~B quality-control summary is shown in figure~\ref{fig:heart-stl}.
...
\begin{figure}[htbp]
    ...
    \caption{Representative Phase~B quality-control summary for case~720. The left panel shows geometry and topology checks (connected components after cleanup, watertight mesh, and median radius in mm) and the right panel shows the printability flag (percentage of skeleton-sample radii below the \SI{0.6}{mm} proxy). This is a single-case computational demonstration and is not presented as evidence of physical printing, perfusion, or biological validation.}
    \label{fig:heart-stl}
\end{figure}

% Figure anonymity/legibility verified for the STL render (figure5); figure6 image is a QC bar chart, not a heart/shell STL.
\TODO{figure~\ref{fig:vessel-stl} (vessel STL) was verified to carry no baked-in file paths, usernames, or identifying text; figure~\ref{fig:heart-stl} likewise carries none. Note that figure~\ref{fig:heart-stl} currently shows a Phase~B QC bar chart, not the heart/shell STL rendering originally intended for this slot, and is largely redundant with figure~\ref{fig:fabrication-readiness-qc}: either regenerate a shell/heart STL render from data or drop this figure. Re-confirm legibility at final print size.}
