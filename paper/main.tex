...
\begin{enumerate}
  ...
  \item Remaining before submission: regenerate figure~\ref{fig:training-curves} (training curves) to the accountable run and test-mean band (its per-epoch history is not in the local archive); restore author names in \texttt{LICENSE}/\texttt{CITATION.cff} post-review. (Figure~\ref{fig:preprocessing} has been regenerated without debug text, and figure~\ref{fig:heart-stl} now shows real test-cohort vessel STLs instead of the redundant QC bar chart.)
...
\begin{figure}[!t]
    ...
    \caption{Validation Dice@0.5 trajectory across A40 fine-tuning. The plot shows the logged per-epoch validation Dice@0.5, a smoothed selected-validation curve and a running-best curve, the best validation checkpoint, and, for reference, the held-out 250-case test mean with its 95\% confidence band.}
    \label{fig:training-curves}
    \TODO{this figure must be regenerated before submission so that (i) the reference test-mean band reads the accountable value 0.7878 (95\% CI 0.7820--0.7935) rather than the retired 0.7953, and (ii) the plotted validation trajectory corresponds to the \texttt{best\_dice05.pt} training run (epoch 79, best validation Dice@0.5 0.7889) that produced the reported test metrics. The currently exported curve is from a different run and does not match either this checkpoint or the v11 history in Section~\ref{subsec:training-convergence}; the full per-epoch history for \texttt{best\_dice05.pt} is not in the local archive.}
\end{figure}
