...
\begin{enumerate}
  ...
  \item Resolve the \textbf{v14 evaluated-checkpoint provenance} (\S\ref{subsec:training-convergence}): the architecture is now proven (strict load, zero key mismatches), but the averaged weights and soup recipe are still not reconstructable --- recover them or re-evaluate from an accountable checkpoint.
  \item \textbf{Full augmentation parameters} for Appendix~\ref{app:repro}.
  \item Proper formatting of the \textbf{WHO reference} in \texttt{references.bib}.
\end{enumerate}}
...
Data augmentation was applied during training. The implementation includes spatial padding, foreground/background random cropping, random flipping, random 90-degree rotation, random spatial rotation, elastic deformation, contrast adjustment, intensity scaling, intensity shifting, Gaussian noise, Gaussian smoothing, and final resize-with-pad-or-crop to the configured patch size. % CHANGED: flag missing magnitudes for reproducibility
\TODO{add the per-transform probabilities and magnitude ranges (e.g. flip $p$, rotation range, elastic $\sigma$/$\alpha$, noise std, intensity-shift offsets) here or in Appendix~\ref{app:repro}; a reviewer will want them for reproducibility.} Validation and inference use sliding-window inference with Gaussian blending to reconstruct full-volume predictions.
...
\TODOBLOCK{Appendix stub. Populate with: (1) the complete augmentation parameter values (per-transform probabilities and magnitude ranges for padding, foreground/background crop ratio, flip, 90$^\circ$ rotation, spatial rotation range, elastic-deformation $\sigma$/$\alpha$, contrast/intensity-scale/intensity-shift ranges, Gaussian-noise std, and Gaussian-smoothing $\sigma$); and (2) either the full per-case results table or an explicit pointer to \texttt{outputs/final\_test\_250/per\_case\_metrics.csv} with a column legend. These values were not available when this draft was assembled.}

\section{Model-provenance record}
\label{app:provenance}

The held-out test metrics were produced by the evaluation command recorded in \texttt{outputs/final\_test\_250/eval\_command.txt}, which loaded the checkpoint \texttt{soup\_ep75-80.pt} and ran sliding-window inference (ROI $96\times192\times192$, overlap 0.625, threshold 0.5) over the 250-case test partition, reporting 0 failed cases and a mean Dice@0.5 of 0.7953. The exported test console log records the loaded model as a MONAI Attention U-Net with channels $(32,64,128,256,512)$, three residual units, dropout 0.1, and 23,625,953 trainable parameters; an independent reconstruction reproduced this parameter count. The evaluation load diagnostics report \texttt{model\_type: attention\_unet} and a strict state-dict load with empty missing- and unexpected-key lists, confirming the evaluated weights matched this architecture. The averaged checkpoint and its epoch-75--80 ingredients were not retained (Section~\ref{subsec:training-convergence}). \TODO{once provenance is resolved, record here the exact soup recipe (ingredient checkpoints and averaging method) or the accountable checkpoint used for re-evaluation, so the evaluated weights are fully reconstructable.}

\newpage
% CHANGED: reminder for the WHO reference formatting (lives in references.bib, not here).
% \cite{who} must have a proper entry: organization=WHO, title="Cardiovascular
% diseases (CVDs)", year, url, urldate. Currently a bare URL. \TODO{fix in references.bib}
