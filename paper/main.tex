...
\begin{figure}[!t]
    ...
    \caption{Spearman correlation between each segmentation metric (rows) and each Phase~B mesh-quality measure (columns) across the 250-case held-out test set. Asterisks denote FDR-corrected $p<0.05$. Topology-aware clDice@0.5 has the strongest (most negative) association with mesh fragmentation (connected-component count, $\rho=-0.49$), whereas wall-thickness compliance is best predicted by precision@0.5 and Dice@0.5.}
...
\begin{figure}[!t]
    ...
    \caption{Qualitative coronary vessel segmentation on three held-out test cases spanning the cohort accuracy range (lower, median, and higher Dice@0.5). Top row: axial maximum-intensity projection (MIP) of the windowed CTA. Bottom row: axial MIP of the segmentation, with ground truth in green, model prediction in red, and their overlap (true positives) in yellow; the predominance of overlap reflects accurate recovery of the main vessel tree, while isolated red and green fragments at the periphery mark the thin distal branches that dominate residual error. Ground truth was resampled onto the prediction's \SI{0.6}{mm} isotropic grid, and the displayed overlap reproduces each case's reported Dice@0.5. These are computational segmentation outputs, not evidence of physical printing or biological validation.}
    \label{fig:qualitative-seg}
\end{figure}

\begin{figure}[!t]
    \centering
    \safeinclude[width=0.56\textwidth]{fig_cldice_vs_dice.png}
    \caption{Per-case topology-aware accuracy (clDice@0.5) versus voxel-overlap accuracy (Dice@0.5) across the 250-case held-out test set. Almost all cases lie above the identity line, indicating that the model preserves vessel centerlines more reliably than full vessel calibre; the cohort mean is $(0.788,\,0.870)$.}
    \label{fig:cldice-vs-dice}
\end{figure}
...
Per-case Dice@0.5 varied across the held-out cohort, with a minimum of 0.6174, maximum of 0.8696, and IQR of 0.0630. The worst cases therefore remained substantially below the cohort mean even though the median approached 0.80. This variability is expected in coronary CTA segmentation because distal branches, reduced contrast, motion artifacts, and annotation uncertainty can disproportionately affect thin vessels. Ranked per-case Dice and the relationship between ground-truth foreground volume and Dice are shown in figures~\ref{fig:ranked-dice} and~\ref{fig:volume-dice}. The cohort distribution of the 95th-percentile Hausdorff distance is shown in figure~\ref{fig:hd95-dist}, and per-case precision against recall (coloured by Dice@0.5) in figure~\ref{fig:precision-recall}; the latter confirms the cohort-wide tendency for recall to exceed precision, indicating mild over-segmentation rather than systematic under-detection.
...
\begin{figure}[!t]
    \centering
    \safeinclude[width=0.72\textwidth]{fig_hd95_distribution.png}
    \caption{Distribution of the 95th-percentile Hausdorff distance (HD95@0.5) across the held-out 250-case test set. The distribution is strongly right-skewed (median \SI{3.69}{mm}, mean \SI{5.01}{mm}): most cases achieve close boundary agreement, while a small number of thin-vessel or distal-branch failures produce large boundary errors.}
    \label{fig:hd95-dist}
\end{figure}

\begin{figure}[!t]
    \centering
    \safeinclude[width=0.62\textwidth]{fig_precision_recall.png}
    \caption{Per-case precision@0.5 versus recall@0.5 across the held-out test set, coloured by Dice@0.5. Most cases lie above the precision\,$=$\,recall diagonal (cohort mean $(0.764,\,0.821)$), confirming a consistent tendency toward recall exceeding precision.}
